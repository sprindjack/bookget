...
func (p *Kyoto) Init(iTask int, sUrl string) (msg string, err error) {
...
func (p *Kyoto) getBookId(sUrl string) (bookId string) {
	m := regexp.MustCompile(`item/([A-z0-9]+)`).FindStringSubmatch(sUrl)
	if m != nil {
		bookId = m[1]
	}
	return bookId
...
func (p *Kyoto) download() (msg string, err error) {
...
func (p *Kyoto) do(imgUrls []string) (msg string, err error) {
...
func (p *Kyoto) getVolumes(sUrl string, jar *cookiejar.Jar) (volumes []string, err error) {
...
func (p *Kyoto) getCanvases(sUrl string, jar *cookiejar.Jar) (canvases []string, err error) {
...
func (p *Kyoto) getBody(sUrl string, jar *cookiejar.Jar) ([]byte, error) {
...
func (p *Kyoto) postBody(sUrl string, d []byte) ([]byte, error) {
	//TODO implement me
	panic("implement me")
}

func (p *Kyoto) doDezoomifyRs(iiifUrls []string) bool {
	if iiifUrls == nil {
		return false
	}
...
func (p *Kyoto) doNormal(imgUrls []string) bool {
	if imgUrls == nil {
		return false
	}
	size := len(imgUrls)
